...
void RandomChanceCards::drawChanceCard()
{
    srand(static_cast<unsigned>(time(0)));
        ...
        if (!chanceDeck.empty()) {
            ...
            moneyChange(drawnCard.moneyChange);
...
void RandomChanceCards::shuffleChanceDeck()
{
    chanceDeck = {
      ...
      { "Invested in the right cryptocurrency. Collect $100.", 100 },
      { "You Have Won In The Casio Collect $200", 200 },
    };
...
void RandomChanceCards::moneyChange(int moneyChanged)
{
	
    
	playerMoney += moneyChanged;
